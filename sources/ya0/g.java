package ya0;

import kotlin.jvm.internal.Intrinsics;
import ya0.d;

/* JADX INFO: compiled from: r8-map-id-9e7ee9a42fe7cc4cda6cf7a37f69f1db09989c88ba665561dcfb218d3d1b2345 */
/* JADX INFO: loaded from: classes7.dex */
public final class g implements d {
    @Override // ya0.d
    public void a(String str, mb0.c preferencesService) {
        Intrinsics.checkNotNullParameter(preferencesService, "preferencesService");
        preferencesService.setReactNativeVersionNumber(str);
    }

    @Override // ya0.d
    public String b(mb0.c cVar) {
        return d.a.b(this, cVar);
    }

    @Override // ya0.d
    public String c(mb0.c preferencesService) {
        Intrinsics.checkNotNullParameter(preferencesService, "preferencesService");
        return preferencesService.V();
    }

    @Override // ya0.d
    public void d(String str, mb0.c preferencesService) {
        Intrinsics.checkNotNullParameter(preferencesService, "preferencesService");
        preferencesService.J(str);
    }

    @Override // ya0.d
    public String e(mb0.c preferencesService) {
        Intrinsics.checkNotNullParameter(preferencesService, "preferencesService");
        return preferencesService.g();
    }

    @Override // ya0.d
    public void f(String str, mb0.c preferencesService) {
        Intrinsics.checkNotNullParameter(preferencesService, "preferencesService");
        preferencesService.setJavaScriptPatchNumber(str);
    }

    @Override // ya0.d
    public void g(String str, mb0.c cVar) {
        d.a.d(this, str, cVar);
    }

    @Override // ya0.d
    public String h(mb0.c preferencesService) {
        Intrinsics.checkNotNullParameter(preferencesService, "preferencesService");
        return preferencesService.f();
    }
}
